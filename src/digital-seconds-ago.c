 ...
 * x: 160-bit private key, x from (0, q). user associated
...
void user_register()
{
	...
	read_input("enter the public key: ", new_user->pubkey, PUBKLEN,
			"fail to read public key");
	if (is_hex(new_user->pubkey) == 0) {
		free(new_user);
		puts("invalid key");
		return;
	}
	read_input("enter the profile: ", new_user->profile, PROFLEN,
			"fail to read profile");
	printf("username: %s\npublic key: %s\nprofile: %s\n", new_user->name,
...
void user_login()
{
	...
	if (u == NULL)
		report_err("fail to find the user");
	if (gen_chall(chall, 16) == -1)
		report_err("fail to create a challenge");
	printf("challenge: %s\n", chall);
...
void user_help()
{
	...
	struct {
		...
		{"pubkey", "list the public key of a user"}
	};
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		if (fgets(cmd, 16, stdin) == NULL)
			report_err("fail to get command");
		remove_new_line(cmd);
